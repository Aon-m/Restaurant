# The Odin Project - Restaurant

This is a solution to the [Restaurant Page Challenge on The Odin Project](https://www.theodinproject.com/lessons/node-path-javascript-restaurant-page). 

## Table of contents

- [The Odin Project - Restaurant](#the-odin-project---restaurant)
  - [Table of contents](#table-of-contents)
  - [Overview](#overview)
    - [The Challenge](#the-challenge)
    - [Links](#links)
    - [Author](#author)
  - [Acknowledgments](#acknowledgments)
    - [Project Continued development](#project-continued-development)
  

## Overview

### The Challenge

Users should be able to:

- Interact with the webpage
- View the optimal layout for each of the website's pages depending on their device's screen size
- See hover states for all interactive elements on the page

### Links

- [Solution URL](https://github.com/Aon-m/Restaurant/)
- [Live Server URL](https://aon-m.github.io/Restaurant/)

### Author

- Frontend Mentor - [@Aon](https://www.frontendmentor.io/profile/Aon-m)
- CSSBattle - [@Aon](https://cssbattle.dev/player/aon)
- Github - [@Aon-m](https://github.com/Aon-m)

## Acknowledgments

- The Crew members featured on the cards are gotten from [BLTX](https://forums.tapas.io/t/character-reference-sheets-are-important-for-any-comic/67362)
- The template for this README file was gotten from [Frontend Mentor](https://www.frontendmentor.io/)
- The background image was gotten from [Pinterest](https://i.pinimg.com/originals/39/26/38/392638526d161a565a88f675fcfc701b.jpg)

### Project Continued development

Additional features, such as menu item images and a shopping cart, may be implemented in future updates.
